...
if (is_null(theme_get_setting('user_notverified_display'))) {
  ...
  $defaults = array(
    ...
    'iepngfix_display'                      => 1,
    'colorswitch_display'                   => 1,
    'suckerfish_display'                    => 1,
    'search_snippet'                        => 1,
    'search_info_type'                      => 1,
    'search_info_user'                      => 1,
    'search_info_date'                      => 1,
...
function phptemplate_preprocess_page(&$vars) {
  ...
  $body_classes[] = 'layout-'. (($vars['sidebar_first'] || $vars['sidebar_second']) ? 'first-main' : 'main') . (($vars['sidebar_last']) ? '-last' : '');  // Page sidebars are active
  ...
  // Add iepngfix on all pages
  if (theme_get_setting('iepngfix_display') == 1) {
    drupal_add_js(drupal_get_path('theme', 'm4music') .'/js/jquery.pngFix.js', 'theme');
  }

  // Add suckerfish on all pages
  if (theme_get_setting('suckerfish_display') == 1) {
      // js is added in page.tpl.php
    drupal_add_css(drupal_get_path('theme', 'm4music') .'/suckerfish.css', 'theme');
  }

  // show colorswitch on all pages
  if (theme_get_setting('colorswitch_display') == 1) {
    //session_start();
    //print_r($_SESSION);
    if (!isset($_SESSION['colorswitch_displaymodus'])) {
...
function phptemplate_menu_item_link($link) {
  ...
  if ($link['depth'] == 1 
      //&&  $user->uid != 1
      ) { // 1 for first level, 2 for second, etc
    return l($link['title'], $link['href'], $link['localized_options']).'<span class="tip">'.$link['localized_options']['attributes']['title'].'</span>';
  }
